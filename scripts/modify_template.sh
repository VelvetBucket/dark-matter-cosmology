...
if [ ! -d "$template_dir" ] && [ ! -d "$original_dir" ]; then
    printf '%s\n' \
        "Neither template_files nor template_filesOG exists:" \
        "$template_dir" \
        "$original_dir" >&2
    exit 1
fi
...
extract_odd_block() {
    ...
    awk '
        /^[[:space:]]*\/\/ BEGIN AUTO ODD PARTICLES[[:space:]]*$/ {
            inside=1
        }
        inside {
            print
        }
        /^[[:space:]]*\/\/ END AUTO ODD PARTICLES[[:space:]]*$/ {
            if (inside) {
                exit
            }
        }
...
restore_clean_template() {
    ...
    if ! cp -a "$original_dir" "$template_dir"; then
        printf '%s\n' \
            "ERROR: failed to restore template_files from template_filesOG." >&2
        printf '%s\n' \
            "template_filesOG was not modified and is still available at:" \
            "$original_dir" >&2
        exit 1
    fi
}
...
fail_modification() {
    printf '%s\n' \
        "Failed to modify template_files. Restoring a clean copy from template_filesOG." >&2

    restore_clean_template

    printf '%s\n' \
        "template_filesOG was preserved unchanged." >&2
    exit 1
}

# If template_files is absent but template_filesOG already exists,
# recreate the working directory from the permanent original.
if [ ! -d "$template_dir" ]; then
    if ! cp -a "$original_dir" "$template_dir"; then
        printf '%s\n' \
            "Failed to recreate template_files from template_filesOG." >&2
        exit 1
    fi
fi
...
# Compare the complete generated odd-sector block with the block expected
# from the current odd_particles.txt. This prevents a template generated
# for model A from being mistaken for the requested model B.
expected_odd_block="${odd_cpp_block%$'\n'}"
current_odd_block="$(
    extract_odd_block "$template_dir/cpp_process_function_definitions.inc" || true
)"

if [ "$current_odd_block" != "$expected_odd_block" ]; then
    is_modified=false
fi
...
if [ "$is_modified" = true ]; then
    printf '%s\n' \
        "template_files is already modified for the requested odd_particles.txt. No changes were made."
    exit 0
fi
...
#
# First execution:
#   template_files   -> template_filesOG
#   template_filesOG -> fresh template_files copy
#
# Later executions (another UFO/model):
#   template_filesOG is NEVER overwritten.
#   The current template_files is discarded and recreated from
#   template_filesOG before applying the new odd-sector information.

if [ ! -d "$original_dir" ]; then
    # First execution. Preserve MadGraph's pristine templates permanently.
    if ! mv "$template_dir" "$original_dir"; then
        printf '%s\n' \
            "Failed to create template_filesOG from the original template_files." >&2
        exit 1
    fi
else
    printf '%s\n' \
        "template_filesOG already exists. Rebuilding template_files from the preserved original."
    rm -rf "$template_dir"
fi

if ! cp -a "$original_dir" "$template_dir"; then
    printf '%s\n' \
        "Failed to create a clean template_files copy from template_filesOG." >&2
    printf '%s\n' \
        "template_filesOG was preserved unchanged at:" \
        "$original_dir" >&2
    exit 1
fi
...
printf '%s\n' \
    "template_files modified successfully using odd_particles.txt."
printf '%s\n' \
    "template_filesOG was preserved unchanged and remains the permanent original."
