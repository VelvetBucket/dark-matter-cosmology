...
double gauss( double (*func)(double, double),double par, double a,double b, int n)
{
  ...
  double ans=0;
  if(n<1) n=1;
  if(n>7) {cout << "7 is a miximum number of points for Gauss integration "<< endl; n=7;} 
  switch(n)
  {  int i;
...
double eps, double * aEps, double * ans, double * aAns, int* N, int depth, int * err)
{
  ...
  r_gauss(func, par,a,(a+b)/2,eps,aEps,ans,aAns,N,depth+1,err);
  if(*err) return;
  r_gauss(func, par,(a+b)/2,b,eps,aEps,ans,aAns,N,depth+1,err);
} 
...
double gauss345( double (*func)(double, double),double par, double a,double b, double eps,int * err_code)
{
  ...
  for(n=0,aEps=0;n<4;n++) aEps+=F4[n]*fabs(func(a+ (b-a)*X4[n], par));
  ...
  {  double ans=0., aAns=0., aEps0=aEps;
     ...
     r_gauss(func,par,a,b,eps,&aEps,&ans,&aAns,&N,0,&err);
...
double eps, double * aEps, double * ans, double * aAns, double _f_ , int depth, int depth1, int*nErr)
{
  ...
  r_simpson(func,par,f,a,(a+b)/2,eps,aEps,ans,aAns,_f_,depth+1, depth1, nErr);
  for(i=0;i<5;i++) f[2*i]=f1[i];
  for(i=1;i<8;i+=2){ f[i]=(*func)((a+b)/2+i*(b-a)/16, par); if(!isfinite(f[i])){ f[i]=0;*nErr=*nErr|1;}} //     *N-=4;
  r_simpson(func,par, f,(a+b)/2,b,eps,aEps,ans,aAns,_f_,depth+1,depth1,nErr);
}
...
double simpson( double (*func)(double, double),double par, double a,double b, double  eps, int *err)
{
  ...
  aEps=0;
  if(a==b) return 0;
  for(i=0;i<9;i++) { f[i]=(*func)(a+i*(b-a)/8, par); if(!isfinite(f[i])){ f[i]=0; nErr=nErr|1;} aEps +=fabs(f[i]); }
...
//printf("SIMPSON: esp=%E aEps=%E\n", eps,aEps);
  ...
  {  double ans=0., aAns=0.;
     ...
     r_simpson(func,par,f,a,b,eps,&aEps,&ans,&aAns,_f_,0,depth1, &nErr);
     
     if(err) *err=nErr;
     if(nErr==0)
     { 
       if(5*aAns*eps > aEps || j>=2 ) return ans;
       for(i=0;i<9;i++) {  f[i]=(*func)(a+i*(b-a)/8, par); if(!isfinite(f[i])){f[i]=0;nErr=nErr|1;}}  // N-=9;
       aEps=aAns*eps;
       continue;
     }
     if(!err && nErr ) 
...
double trapezoidal(double (*func)(double, double),double par, double a, double b, double  eps) {
    ...
    int n = 1, i_max=24;
    double h = (b - a) / n;           
    double s_new = 0.5 * h * (func(a,par) + func(b,par)), s_old, term;
    
    if (eps < 1e-08){eps = 1e-08;}
    s_old = 2*eps + s_new;
    ...
    for  (int i = 1; i < i_max; i++){
      ...
      h = h/2;
      for (int j = 1; j < n; j+=2) {
        term += func(a + j * h,par);        // Add middle terms
      }
      s_new = 0.5*s_old + h*term;
      if (fabs(s_old - s_new) < fabs(s_new)*eps || fabs(s_new) < 1e-50){
        return s_new;
      }
    }
    printf("Trapezoidal:: Max iterations reached. Maybe not accurate.");
    return s_new;               // Return integral approximation
}
...
double romberg(double (*f)(double, double), double par, double a, double b, double acc) 
{
  ...
  Rp[0] = (f(a, par) + f(b, par))*h*0.5; // first trapezoidal step
...
std::vector<std::pair<int, int>> findPeakWidth(const std::vector<double> data, std::vector<int> peakIndexes) {
    double threshold;
    int left, right;
    std::vector<pair<int, int>> limits;
    
    for (int peakIndex : peakIndexes){
      threshold = data[peakIndex] / 2.0;
      left = peakIndex;
      right = peakIndex;
      
      // Find left edge
      while (left > 0 && data[left] >= threshold) --left;
      // Find right edge
      while (right < data.size() - 1 && data[right] >= threshold) ++right;
      
      limits.push_back({left, right});
    } 
    return limits; // Returns start and end indices of width
}
